...
async function main(): Promise<void> {
  ...
  console.log(
    `  Input: ${allOfferings.length} offerings, ${timeSlots.length} time slots`,
  );
  ...
  for (const { offering, failedCheck } of validation.infeasible) {
    console.log(
      `    ❌ ID=${offering.id} "${offering.course.name}" → [${failedCheck.code}]`,
    );
  }

  const fixedCount = candidates.filter((c) => c.isFixedRoom).length;
  console.log(
    `  Candidates: ${candidates.length} (${fixedCount} fixed, ${candidates.length - fixedCount} flexible)`,
  );
  ...
  for (const l of lecturers) {
    const prefSlots =
      l.preferredTimeSlotIds.length > 0
        ? l.preferredTimeSlotIds
            .map((sid) => {
            ...
            .join(", ")
        : "(any — fully flexible)";
    const tags = [l.isStructural ? "🏛️ structural" : ""]
      .filter(Boolean)
      .join(" ");
  ...
  for (const crossoverType of crossoverTypes) {
    ...
    console.log(
      `    Status:            ${gaResult.hardViolations === 0 ? "✅ VALID" : "⚠️  CONFLICTS"}`,
    );
    ...
    if (crossoverType === "pmx") {
      ...
      const fixedGenes = gaResult.bestChromosome.filter(
        (g) => g.kind === "FIXED",
      );
      ...
      for (const gene of gaResult.bestChromosome) {
        ...
        const candidate = candidates.find(
          (c) => c.offeringId === gene.offeringId,
        );
        const expectedDuration =
          candidate?.sessionDuration ?? offering.course.sks;

        const lecNames = offering.lecturers
          .map((l) => l.name.split(" ")[0])
          .join("+");
        const prefCheck = offering.lecturers
          .map((l) => {
            const pref = lecturerPreferenceMap.get(l.id);
            if (!pref || pref.size === 0) return "🔵";
            return gene.sessions.every((s) =>
              s.timeSlotIds.every((sid) => pref.has(sid)),
            )
              ? "🟢"
              : "🟡";
          })
        ...
        for (const s of gene.sessions) {
          if (s.timeSlotIds.length > longestBlock)
            longestBlock = s.timeSlotIds.length;
          if (!isContiguous(s.timeSlotIds, slotLookup)) nonContiguousCount++;
          if (s.timeSlotIds.length !== expectedDuration)
  ...
  function buildLongSessionScenario(): {
    ...
    const longCourses = [
      ...
      },
      {
        id: 3,
        code: "LS302",
        name: "Lab Practicum (3-slot)",
        sks: 3,
        requiredFacilities: ["LAB"],
        requiredCompetencies: ["lab"],
      },
    ];
    ...
    const longOfferings: CourseOffering[] = [
      {
        id: 101,
        courseId: 1,
        course: longCourses[0]!,
        roomId: 1,
        room: longRooms[0]!,
        lecturers: [longLecturers[0]!],
      ...
      },
      {
        id: 102,
        courseId: 2,
        course: longCourses[1]!,
        roomId: 2,
        room: longRooms[1]!,
        lecturers: [longLecturers[1]!],
      ...
      },
      {
        id: 103,
        courseId: 3,
        course: longCourses[2]!,
        roomId: 3,
        room: longRooms[2]!,
        lecturers: [longLecturers[2]!],
    ...
    return {
      rooms: longRooms,
      timeSlots: longTimeSlots,
      lecturers: longLecturers,
      offerings: longOfferings,
    };
  }
  ...
  console.log(
    `  Grid:      ${demo.timeSlots.length} slots (5 days × 4 back-to-back hours)`,
  );
  console.log(`  Offerings:`);
  for (const o of demo.offerings) {
    console.log(
      `    • ${o.course.code} "${o.course.name}" → sks=${o.course.sks} (must occupy ${o.course.sks} contiguous hours)`,
    );
  }
  console.log();
  ...
  if (!demoGA) {
    ...
    for (const gene of demoGA.bestChromosome) {
      ...
      console.log(
        `    ${offering.course.code} "${offering.course.name}" (sks=${offering.course.sks})`,
      );
      ...
      for (const s of gene.sessions) {
        if (s.timeSlotIds.length > demoLongest)
          demoLongest = s.timeSlotIds.length;
        ...
        console.log(
          `           ↳ raw: [${s.timeSlotIds.join(",")}] = ${formatSession(s.timeSlotIds, demoSlotLookup)}`,
        );
      }
    }
    ...
    console.log(
      `    Longest contiguous block:  ${demoLongest} slot(s) ${demoLongest >= 4 ? "✅" : demoLongest === 3 ? "✅ (3 ok)" : "❌"}`,
    );
    console.log(
      `    Non-contiguous sessions:   ${demoNonContig} ${demoNonContig === 0 ? "✅" : "❌"}`,
    );
    console.log(
      `    Duration mismatches:       ${demoMismatch} ${demoMismatch === 0 ? "✅" : "❌"}`,
    );
    const ok = demoLongest >= 4 && demoNonContig === 0 && demoMismatch === 0;
    console.log(
      `    Long-session support:      ${ok ? "✅ VERIFIED end-to-end" : "⚠️  see warnings above"}`,
    );
  }
  ...
  console.log(
    `  Layer 3 (GA):      3 crossover runs above + 1 long-session demo`,
  );
