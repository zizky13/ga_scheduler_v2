...
Things to verify or fill in before this README is considered complete:

- **`TODO`: license** — `package.json` declares `ISC`, but there is no `LICENSE` file at the repo root. Add one (or change the field) to make the licensing intent explicit.
- **`TODO`: author / repository metadata** — `package.json` has empty `author`, `description`, and `keywords` fields and no `repository` URL.
- **`TODO`: tests** — the `test` npm script is a stub (`echo "Error: no test specified" && exit 1`). The tech spec implies a test strategy; pick a runner (e.g. `vitest`, `node --test`) and wire it up.
- **`TODO`: production data source** — `src/db/seed.ts` is mock data. Per the spec, real inputs will eventually come from a Prisma-backed SQLite/libSQL database via the `LockedRoom` and offering tables. That integration is not in this repository yet.
- **`TODO`: API / persistence layer** — the spec describes Express + Redis for long-running GA jobs; this repository contains the pure computation core only.
