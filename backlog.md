...
**Core shared components (only what the critical path consumes)**
...
72. [ ] `[P1/M]` Implement **WCAG AA keyboard navigation**: all interactive elements focusable via Tab, focus order follows visual order, sidebar navigable with arrow keys, modal focus trapping with return-focus on close, `:focus-visible` ring using `--shadow-ring` (design-spec §15).
73. [ ] `[P1/M]` Add **semantic HTML and ARIA attributes**: `<nav aria-label>` for sidebar with `aria-current="page"`, `aria-expanded` on collapse toggle, `<table>` with `<caption>` and `aria-sort` on sortable headers, `role="dialog"` + `aria-modal` + `aria-labelledby` on modals, `role="alert"` + `aria-live` on toasts, `role="progressbar"` + `aria-valuenow/min/max` on progress bar, `role="grid"` + `aria-label` on schedule grid with `role="gridcell"` + descriptive `aria-label` on course blocks, text summary below fitness chart for screen readers (design-spec §15).
