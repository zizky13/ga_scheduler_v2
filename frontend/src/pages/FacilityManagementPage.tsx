...
function CountCell({ count, facilityCode, items, popoverTitle, viewAllRoute, viewAllLabel }: CountCellProps) {
    ...
    async (p: number, ps: number) => {
      ...
      try {
        ...
        const enriched: FacilityWithCounts[] = facRes.data.map((fac) => {
          const matchingRooms = roomsRes.data.filter((r) =>
            r.facilities.includes(fac.code),
          );
    ...
    ? (() => {
        ...
        if (deleteTarget.roomCount > 0 || deleteTarget.courseCount > 0) {
          ...
          if (deleteTarget.courseCount > 0)
            parts.push(`• ${deleteTarget.courseCount} course${deleteTarget.courseCount > 1 ? 's' : ''}`);
