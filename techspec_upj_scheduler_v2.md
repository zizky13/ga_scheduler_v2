...
- Every lecturer carries a `maxSks` cap (default 12 non-structural / 6 structural; `0` = on leave). The GA's `loadPenalty` softly penalizes any lecturer whose total assigned `course.sks` across the semester exceeds their `maxSks`. Distinct from `structuralPenalty`: that measures weekly scheduling density (sessions/week) for structural roles only; `loadPenalty` measures semester-wide teaching volume (SKS sum) for every lecturer. Both ride the shared `softPenaltyWeight`.
